...
float total_R, total_L, total_RL;       //total distance covered of right and left and sum of both

//......RIGHT MOTOR.......//
float distance_R = 1 ;                     //DISTANCE MEASUREMENT 1=cm, 100=meters, 1000=km
float radius_R = 3.35/distance_R ;        //diameter in terms of "cm" (Put in cm always)
float circumference_R = 3.142 * radius_R * distance_R ;
float velocity_time_R = 10 ;                //10ms
float change_distance_R = 0 ;               //do not alter this line
float new_time_R = 0, old_time_R = 0, change_time_R = 1 ;   //do not alter this line
float velocity_R = 0 ;
...
float kp_R = 13.0, ki_R = 0.0, kd_R = 0.55;  //  tuning parameter
float error_R = 0, sum_error_R = 0, last_error_R = 0;
float pid_R = 0;  //  pid computation result
int set_point_R = 15;  //  desired velocity in terms of cm/s
int dt_R = velocity_time_R;

//......LEFT MOTOR PID Parameters......//

float kp_L = 14.6, ki_L = 0.0, kd_L = 0.6;  //  tuning parameter
float error_L = 0, sum_error_L = 0, last_error_L = 0;
float pid_L = 0;  //  pid computation result
int set_point_L = 18.3;  //  desired velocity in terms of cm/s
...
velocity_R = get_velocity_R();  //Get velocity
Right_Forward();                //Motor direction 
...
float get_velocity_R(void){
  ...
  if((new_time_R - old_time_R) >= velocity_time_R){
    ...
    error_R = set_point_R-velocity_R;  //  calculating Error
    sum_error_R += error_R;  //  summation of error
    pid_R = kp_R*error_R + ki_R*sum_error_R + kd_R*(error_R-last_error_R)/dt_R;  //  PID result value(OUTPUT)
    last_error_R = error_R;
    pid_R = constrain(pid_R,40,255);  //  defines limit
    }
    return velocity_R;
  }
...
float get_distance_R(void){
  ...
  change_distance_R1*= circumference_R;  //converting revolution to cm
...
float get_velocity_L(void){
  ...
  if((new_time_L - old_time_L) >= velocity_time_L){
    ...
    error_L = set_point_L-velocity_L;  //  calculating Error
    sum_error_L += error_L;  //  summation of error
    pid_L = kp_L*error_L + ki_L*sum_error_L + kd_L*(error_L-last_error_L)/dt_L;  //  PID result value(OUTPUT)
    last_error_L = error_L;
    pid_L = constrain(pid_L,40,255);  //  defines limit
    }
    return velocity_L;
  }
...
float get_distance_L(void){
  ...
  change_distance_L1*= circumference_L;  //converting revolution to cm
